...
const TransactionsHistory = ({ items }) => {
  const history = items.map(({ id, type, amount, currency }, index) => {
    ...
    return <tr className={stringColor.join(" ")} key={id}>
...
TransactionsHistory.propTypes = {
  items: PropTypes.arrayOf(PropTypes.shape({id: PropTypes.string, type: PropTypes.string, amount: PropTypes.string, currency: PropTypes.string}))
}